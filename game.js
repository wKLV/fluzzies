CoolGame.start = function(){
    ...
    labels.add(nballslabel).add(ballsSavedlabel).add(ntoucheslabel).add(tocheslabel).
        add(pass).add(star1).add(star2).add(star3).add(resetlabel).add(timeplus);
    ...
    $.getJSON("map"+params[1]+".js", function(data){
        ...
        basein = (new Sprite).fill("assets/in.svg").size(150, 150).moveTo(data.catcher.position[0], data.catcher.position[1])
        layer.add(base);
        layer.add(basein);
        ...
        basein.event(["mousedown", "touchstart"], function(e){
            if(passed) window.location.href = params[0]+ "?" + (parseInt(params[1])+1)
        })
        bain = basein, basein = data.catcher;
        checkStars = function(){
            if(nballs >= basein.pass.coolies && (ntouches <= basein.pass.touches || typeof basein.pass.touches === "undefined")){
                passed = true; bain.fill("assets/in-open.svg"); pass.fill("assets/doorgot.svg");
            }if(nballs >= basein.star1.coolies && (ntouches <= basein.star1.touches || typeof basein.star1.touches === "undefined")){
               star1.fill("assets/stargot.svg");
            }if(nballs >= basein.star2.coolies && (ntouches <= basein.star2.touches || typeof basein.star2.touches === "undefined")){
               star2.fill("assets/stargot.svg");
            }if(nballs >= basein.star3.coolies && (ntouches <= basein.star3.touches || typeof basein.star3.touches === "undefined")){
                star3.fill("assets/stargot.svg");
            }
        }
...
i
    ...
    listener.PreSolve = function(a,b){
        if(a.GetUserData().name === "cooly" && ! deleted[a.GetUserData().id]){
            if(b.GetUserData().name === "spikes")
                ...
                if(!passed){
                    b.GetUserData().visual.fill("assets/in-open.svg")
                    setTimeout(function(){b.GetUserData().visual.fill("assets/in.svg")}, 1500);
                }
            ...
            if(a.GetUserData().name === "spikes")
                ...
                if(!passed){
                    a.GetUserData().visual.fill("assets/in-open.svg")
                    setTimeout(function(){a.GetUserData().visual.fill("assets/in.svg")}, 500);
                }
    ...
    lime.scheduleManager.schedule(function(dt) {if(ready){
        ...
        for(var i=0; i< steps; i++){
            ...
            var c = emitter.getNext(dt*speed);
            if(c){
